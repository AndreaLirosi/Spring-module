package com.example.Custom.Queries2.controllers;

import com.example.Custom.Queries2.entity.Flight;
import com.example.Custom.Queries2.entity.TypeFlight;
import com.example.Custom.Queries2.repository.Repository_Flight;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.data.domain.Sort;
import org.springframework.web.bind.annotation.*;

import java.util.ArrayList;
import java.util.List;
import java.util.Random;

@RestController
@RequestMapping("/Fly")
public class FlightController {

    @Autowired
    private Repository_Flight repositoryFlight;

    @GetMapping("/get")
    public List<Flight> getAllFlights() {
        return repositoryFlight.findAll();
    }
    @GetMapping("/getByPage")
    public List<Flight> getAllByPageFlights(@RequestParam(value = "page",defaultValue = "1") int page, @RequestParam(value = "size",defaultValue = "5") int size) {
        Pageable paginazione = PageRequest.of(page,size, Sort.by("fromAirport").ascending());
        return repositoryFlight.findAll(paginazione).toList();
    }
    @GetMapping("/Ontime")
    public List<Flight> getFLyOntime() {
        return repositoryFlight.findFlightByTypeFlight(TypeFlight.ONTIME);
    }

    @GetMapping("/getCose")
    public List<Flight> getFlyCose(@RequestParam(value = "stato1") String bob1, @RequestParam(value = "cosa2") String bob2) {
        return repositoryFlight.findFlyCose(translateStatus(bob1), translateStatus(bob2));
    }

    @PostMapping("/provision")
    //andiamo a creare una lista di voli
    public List<Flight> provisionFlights() {
        List<Flight> flights = new ArrayList<>();
        Random random = new Random();
        //andiamo a generare oggetti di tipo flight, i nostri voli, in modo casuale
        for (int i = 0; i < 10; i++) {
            Flight flight = new Flight();
            //usiamo un generatore casuale di stringe per la descrizione e altri parametri
            flight.setDescription(generateRandomString());
            flight.setFromAirport(generateRandomString());
            flight.setToAirport(generateRandomString());
            // li settiamo tutti su ONTIME e li aggiungiamo
            flight.setTypeFlight(generateRandomEnum());
            flights.add(flight);
        }
        // questo ci permette di salvarli
        return repositoryFlight.saveAll(flights);
    }

    // per generare parole casuali si usa random.ints.
    private String generateRandomString() {
        // qui si pongono i limiti che vanno da 97 a 122 (valori ASCII delle lettere minuscole) e si pone la lunghezza della parola
        int leftLimit = 97; // letter 'a'
        int rightLimit = 122; // letter 'z'
        int targetStringLength = 10;
        Random random = new Random();// chiamiamo la funzione Random
        // random int genera una sequenza di numeri interi casuali
        return random.ints(leftLimit, rightLimit + 1)
                .limit(targetStringLength)// impostiamo il limite
                // collect raccoglie gli elementi del flusso in una struttura specifica (stringBuilder) il quale fornisce un nuovo oggetto e raccoglie i numeri e append va ad'unire il risultato precedente con quello successivo
                .collect(StringBuilder::new, StringBuilder::appendCodePoint, StringBuilder::append)
                //infine viene trasformato in stringa
                .toString();
    }

    private TypeFlight generateRandomEnum() {
        Random random = new Random();
        int random_int = random.nextInt(0, 3);
        switch (random_int) {
            case 1:
                return TypeFlight.CANCELLED;
            case 2:
                return TypeFlight.DELAYED;
            default:
                return TypeFlight.ONTIME;
        }
    }

    private TypeFlight translateStatus(String status) {
        return status.equals("ONTIME") ? TypeFlight.ONTIME : status.equals("DELAYED") ? TypeFlight.DELAYED : TypeFlight.CANCELLED;
    }
}
